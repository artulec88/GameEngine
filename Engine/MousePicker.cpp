...
void Engine::MousePicker::CalculateCurrentRay(double xPos, double yPos, const Math::Matrix4D& projectionMatrix, const Math::Matrix4D& viewMatrix)
{
	...
	DEBUG_LOG_ENGINE("Normalised device space mouse position for viewport coordinates (%.2f, %.2f) = %s", xPos, yPos, normalisedDeviceSpaceRay.ToString().c_str());
	...
	DEBUG_LOG_ENGINE("Homogeneous clip space mouse position = %s", homogeneousClipCoordinatesRay.ToString().c_str());
	...
	DEBUG_LOG_ENGINE("Eye space ray = %s", eyeSpaceRay.ToString().c_str());
	...
	DEBUG_LOG_ENGINE("World space ray for position (%.2f, %.2f) = %s", xPos, yPos, worldSpaceRay.ToString().c_str());
}