...
void Engine::MousePicker::CalculateCurrentRay(double xPos, double yPos, const Math::Matrix4D& projectionMatrix, const Math::Matrix4D& viewMatrix)
{
	...
	DEBUG_LOG_ENGINE("Normalised device space mouse position for viewport coordinates (", xPos, ", ", yPos, ") = ", normalisedDeviceSpaceRay.ToString());
	...
	DEBUG_LOG_ENGINE("Homogeneous clip space mouse position = ", homogeneousClipCoordinatesRay.ToString());
	...
	DEBUG_LOG_ENGINE("Eye space ray = ", eyeSpaceRay.ToString());
	...
	DEBUG_LOG_ENGINE("World space ray for position (", xPos, ", ", yPos, ") = ", worldSpaceRay.ToString());
}