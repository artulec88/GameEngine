...
{
	CHECK_CONDITION_ALWAYS_MATH_TEST(expectedCompareResult || expectedLessResult || expectedGreaterResult, Utility::ERR, "Incorrect setup of the test. One of the values for comparing angles must be true.");
	CHECK_CONDITION_ALWAYS_MATH_TEST(!expectedLessResult || !expectedGreaterResult, Utility::ERR, "Incorrect setup of the test. Any given angle cannot be both smaller and greater than some other angle.");
	if (expectedCompareResult)
	{
		CHECK_CONDITION_ALWAYS_MATH_TEST(!expectedLessResult && !expectedGreaterResult, Utility::ERR, "Incorrect setup of the test. Expected ==, < and > operators result: %d, %d, %d are incorrect.",
...
void AngleTestCompare::StartTest()
{
	...
	CHECK_CONDITION_ALWAYS_MATH_TEST(lessResult == m_expectedLessResult, Utility::ERR,
		"The angle %s was expected to be smaller than angle %s but it is not.",
		m_angle.ToString().c_str(), m_compareAngle.ToString().c_str());

	bool greaterResult = (m_angle > m_compareAngle);
	CHECK_CONDITION_ALWAYS_MATH_TEST(greaterResult == m_expectedGreaterResult, Utility::ERR,
		"The angle %s was expected to be greater than angle %s but it is not.",
		m_angle.ToString().c_str(), m_compareAngle.ToString().c_str());

	CHECK_CONDITION_ALWAYS_MATH_TEST(!lessResult || !greaterResult, Utility::ERR,
