...
CubeShadowMap::CubeShadowMap(void) :
	m_fbo(0),
	m_renderBuffer(0),
	m_shadowMap(0),
	m_depth(0)
...
void CubeShadowMap::Init(unsigned int windowWidth, unsigned int windowHeight)
{
	...
	INFO_LOG_RENDERING("Initializing cube shadow map with width=%d and height=%d (cubeMapSize=%d)", windowWidth, windowHeight, cubeMapSize);
