...
public:
	inline void SetVector3D(const std::string& name, const Math::Vector3D& vec)
	{
		...
		{
			stdlog(Utility::Debug, LOGPLACE, "Vector3D with name \"%s\" is not found in the map. Creating a new vector with this name.", name.c_str());
			vec3DMap.insert(std::pair<std::string, Math::Vector3D>(name, vec));
		}
		else
		{
#ifdef _DEBUG
			stdlog(Utility::Delocust, LOGPLACE, "Modifying the Vector3D \"%s\" with values \"%s\" to vector \"%s\"", name.c_str(), vec3DMap[name].ToString().c_str(), vec.ToString().c_str());
#endif
			vec3DMap[name] = vec;
		}
	}
	...
	inline void SetReal(const std::string& name, const Math::Real value)
	{
		...
		{
			stdlog(Utility::Debug, LOGPLACE, "The real value with name \"%s\" is not found in the map. Creating a new real value with this name.", name.c_str());
			realMap.insert(std::pair<std::string, Math::Real>(name, value));
		}
		else
		{
#ifdef _DEBUG
			stdlog(Utility::Delocust, LOGPLACE, "Modifying the real value with name \"%s\" from \"%.2f\" to \"%.2f\"", name.c_str(), realMap[name], value);
#endif
			realMap[name] = value;
		}
	}
	...
	inline void SetTexture(const std::string& textureName, Texture* texture)
	{
		...
		{
			stdlog(Utility::Debug, LOGPLACE, "The texture with name \"%s\" is not found in the map. Creating a new texture with this name.", textureName.c_str());
			textureMap.insert(std::pair<std::string, Texture*>(textureName, texture));
		}
		else
		{
#ifdef _DEBUG
			stdlog(Utility::Delocust, LOGPLACE, "Modifying the texture with name \"%s\".", name.c_str());
#endif
