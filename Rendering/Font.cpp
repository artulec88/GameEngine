...
void Rendering::Text::Font::ReadMetaDataFile(const std::string& fontMetaDataFileName)
{
	...
	CHECK_CONDITION_RETURN_VOID_RENDERING(file.is_open(), Utility::Error, "Could not open font meta data file \"%s\"", fontMetaDataFileName.c_str());
	...
	while (!file.eof())
	{
		...
		if (tokens[0].compare("info") == 0)
		{
			...
			CHECK_CONDITION_RENDERING(tokenItr != tokens.end(), Utility::Error,
			...
			CHECK_CONDITION_RENDERING(subTokens.size() == 4, Utility::Error, "The \"padding\" attribute must define 4 values instead of %d", subTokens.size());
...
const Rendering::Text::Character& Rendering::Text::Font::GetCharacter(int asciiCode) const
{
	...
	CHECK_CONDITION_RENDERING(characterItr != m_metaData.end(), Utility::Error,
