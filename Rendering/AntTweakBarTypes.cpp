...
/* static */ void Rendering::AntTweakBarTypes::InitializeTweakBarTypes()
{
	...
	TwEnumVal angleUnitEV[2] = { { Math::Angle::Unit::DEGREE, "Degree" }, { Math::Angle::Unit::RADIAN, "Rad" } };
	angleUnitType = TwDefineEnum("Unit", angleUnitEV, 2);
	TwStructMember angleMembers[2] = { { "Angle", TW_TYPE_FLOAT, 0, " precision=1 "}, { "Unit", angleUnitType, 4, ""} };
	angleType = TwDefineStruct("Angle", angleMembers, 2, sizeof(Math::Angle), NULL, NULL);

	TwStructMember vector3DMembers[3] = {
		{ "X", TW_TYPE_FLOAT, 0, " precision=2 "},
		{ "Y", TW_TYPE_FLOAT, 4, " precision=2 "},
		{ "Z", TW_TYPE_FLOAT, 8, " precision=2 "}
	};
	vector3DType = TwDefineStruct("Vector", vector3DMembers, 3, sizeof(Math::Vector3D), NULL, NULL);

	TwStructMember matrix4DRowMembers [4] = {
		{ "[0]", TW_TYPE_FLOAT, 0, " precision=2 " },
		{ "[1]", TW_TYPE_FLOAT, 16, " precision=2 " },
		{ "[2]", TW_TYPE_FLOAT, 32, " precision=2 " },
		{ "[3]", TW_TYPE_FLOAT, 48, " precision=2 " },
	};
