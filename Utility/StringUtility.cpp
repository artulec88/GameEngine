...
{
	auto result = str;
	for (auto i = static_cast<int>(result.length()); i < length; ++i)
	{
		result += padChar;
	}
	return result;
...
int utility::string_utility::ToInt(const std::string& str)
{
	...
	const auto isPositive = inString[0] != '-';
...
void utility::string_utility::CutToTokens(const std::string& str, std::vector<std::string>& tokens, const char* delim, const int delimCount)
{
	...
	const auto quoteSign = '"';
	const char* cstr = str.c_str();
	const auto strLength = static_cast<unsigned int>(str.length());
	auto start = 0;
	auto end = 0;
