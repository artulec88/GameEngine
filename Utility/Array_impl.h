...
T& Utility::Container::Array<T>::operator[](int i)
{
	...
	CHECK_CONDITION_RETURN_ALWAYS_UTILITY(i >= 0, m_data[0], Logging::ERR, "The array [] operator cannot access index ", i, " which is outside of range [0; ", m_size, ")");
	CHECK_CONDITION_RETURN_ALWAYS_UTILITY(i < m_size, m_data[m_size - 1], Logging::ERR, "The array [] operator cannot access index ", i, " which is outside of range [0; ", m_size, ")");
	return m_data[i];
}
...
void Utility::Container::Array<T>::Append(const T& dataElement)
{
	...
	if (m_size == m_maxSize)
	{
		WARNING_LOG_UTILITY("The array cannot hold more than ", m_maxSize, " elements");
	}
...
{
	WARNING_LOG_UTILITY("This function has not been tested yet");
	CHECK_CONDITION_RETURN_VOID_ALWAYS_UTILITY(i >= 0, Logging::ERR, "Cannot set array's element at index ", i, ". The correct index must be in the range [0; ", m_size, ")");
	CHECK_CONDITION_RETURN_VOID_ALWAYS_UTILITY(i < m_maxSize, Logging::WARNING, "The array cannot hold more than ", m_maxSize, " elements");
...
{
	WARNING_LOG_UTILITY("This function has not been tested yet");
	CHECK_CONDITION_RETURN_VOID_ALWAYS_UTILITY(i >= 0, Logging::ERR, "Cannot remove array's element at index ", i, ". The correct index must be in the range [0; ", m_size, ")");
	CHECK_CONDITION_RETURN_VOID_ALWAYS_UTILITY(i < m_size, Logging::WARNING, "Cannot remove array's element at index ", i, ". The correct index must be in the range [0; ", m_size, ")");
