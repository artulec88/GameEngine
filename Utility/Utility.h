...
#ifdef _DEBUG
#define CHECK_CONDITION(expr, moduleName, logLevel, ...) do { ASSERT(expr); if (!(expr)) { Utility::Logging::ILogger::GetLogger(moduleName).Log(logLevel, LOGPLACE, ##__VA_ARGS__); } } while (0)
#define CHECK_CONDITION_ALWAYS(expr, moduleName, logLevel, ...) do { ASSERT(expr); if (!(expr)) { Utility::Logging::ILogger::GetLogger(moduleName).Log(logLevel, LOGPLACE, ##__VA_ARGS__); } } while (0)
#define CHECK_CONDITION_EXIT(expr, moduleName, logLevel, ...) do { ASSERT(expr); if (!(expr)) { Utility::Logging::ILogger::GetLogger(moduleName).Log(logLevel, LOGPLACE, ##__VA_ARGS__); system("pause"); exit(EXIT_FAILURE); } } while (0)
#define CHECK_CONDITION_EXIT_ALWAYS(expr, moduleName, logLevel, ...) do { ASSERT(expr); if (!(expr)) { Utility::Logging::ILogger::GetLogger(moduleName).Log(logLevel, LOGPLACE, ##__VA_ARGS__); system("pause"); exit(EXIT_FAILURE); } } while (0)
#define CHECK_CONDITION_RETURN_VOID(expr, moduleName, logLevel, ...) do { ASSERT(expr); if (!(expr)) { Utility::Logging::ILogger::GetLogger(moduleName).Log(logLevel, LOGPLACE, ##__VA_ARGS__); return; } } while (0)
#define CHECK_CONDITION_RETURN_VOID_ALWAYS(expr, moduleName, logLevel, ...) do { ASSERT(expr); if (!(expr)) { Utility::Logging::ILogger::GetLogger(moduleName).Log(logLevel, LOGPLACE, ##__VA_ARGS__); return; } } while (0)
#define CHECK_CONDITION_RETURN(expr, returnValue, moduleName, logLevel, ...) do { ASSERT(expr); if (!(expr)) { Utility::Logging::ILogger::GetLogger(moduleName).Log(logLevel, LOGPLACE, ##__VA_ARGS__); return returnValue; } } while (0)
#define CHECK_CONDITION_RETURN_ALWAYS(expr, returnValue, moduleName, logLevel, ...) do { ASSERT(expr); if (!(expr)) { Utility::Logging::ILogger::GetLogger(moduleName).Log(logLevel, LOGPLACE, ##__VA_ARGS__); return returnValue; } } while (0)
