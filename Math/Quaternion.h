...
namespace Math
{
	...
	public:
		...
		MATH_API Quaternion(Quaternion&& q) : m_x(std::move(q.m_x)), m_y(std::move(q.m_y)), m_z(std::move(q.m_z)), m_w(std::move(q.m_w))
		{
		}

		MATH_API Quaternion& operator=(const Quaternion& q);
		MATH_API Quaternion& operator=(Quaternion&& q);
	...
	public:
		...
		/// </summary>
		MATH_API inline Quaternion Conjugate() const { return Quaternion(-m_x, -m_y, -m_z, m_w); };
