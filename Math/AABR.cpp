...
{
	CHECK_CONDITION(!(m_bottomLeftPos.GetX() > m_topRightPos.GetX()), Utility::Error,
	...
	CHECK_CONDITION(!(m_bottomLeftPos.GetY() < m_topRightPos.GetY()), Utility::Error,
...
Math::IntersectInfo Math::AABR::DoesContainPoint(Math::Real x, Math::Real y) const
{
	...
	//DELOCUST_LOG("DistanceX = %.3f, distanceY = %.3f", distanceX, distanceY);
